...
if (length(TPS_FEATURESM)>3) % TPS (Thin Plate Spline)
    ...
    [ftps] = TEETH_calc_tps(TPS_FEATURESM,TPS_FEATURESN-TPS_FEATURESM); %% Warning: Matrix is singular to working precision. 
    pt = tP + TEETH_eval_tps(ftps,tP);
    TextureCoords1 = DISCtoPLANE(pt,'p2d')'; %% after this step, regIdx & newTHETA are empty
