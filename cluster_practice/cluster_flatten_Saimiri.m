...
%%% setup paths
base_path = [pwd '/'];
data_path = '../Saimiri/'; %% path to data needed
meshes_path = [data_path 'off']; %% path to .off files
samples_path = [base_path 'samples/Project2/']; %% not sure yet what this leads to...
...
%%% load taxa codes
taxa_file = [data_path 'saimiri_taxa_table.mat']; %% path to names
...
for k = 1:GroupSize
    ...
    if (exist([samples_path taxa_code{k} '.mat'], 'file'))%%%%%%% file name format
        job_id = job_id+1;
        continue;
    end
    ...
    mesh_file= [meshes_path taxa_code{k} '.off']; %%%%%%% file name format
    sample_file= [samples_path taxa_code{k} '.mat']; %%%%%%% file name format
