...
if strcmpi(Type,'MST')
    DistMatrix = sparse(DistMatrix); %sparse only stores nonzero entries
    DistMatrix = tril(DistMatrix,-1); %extract lower triangular part
    [ST, PRED] = graphminspantree(DistMatrix,'Method','Kruskal'); %ST is sparse matrix representing MST (gives nodes and edge length between them), PRED is vector with length equal to number of nodes and entry equal to predecessor in MST
